# comparing the proportion of Treg to other T and NK cells in sensitive and resistant patients.


cluster_proportion_ratio_plot_tnk <- bb_cellmeta(cds_main) %>%
  filter(partition_assignment  %in% c("T", "NK")) |> 
  count(patient, sample, seurat_l2_leiden_consensus, patient_type2, timepoint_merged_1) |> 
  pivot_wider(names_from = seurat_l2_leiden_consensus, values_from = n, values_fill = 1) %>%
  mutate(ratio = (`Treg`/(NK + `CD8 TEM` + `CD4 TCM` + `CD4 Naive`))) %>%
  ...
  labs(y = "log<sub>2</sub>(T<sub>reg</sub>:other)", 
       color = NULL, 
       fill = NULL, 
       x = NULL) +
...
cluster_proportion_ratio_plot_tnk
