...
abs_cd8tem_ibs <- bb_cellmeta(cds_main) |>
  filter(seurat_l2_leiden_consensus  %in% c("CD8 TEM")) |>
  count(patient,
        sample,
        leiden_enrichment,
        patient_type2,
        timepoint_merged_1,
        timepoint_merged_2) |>
  pivot_wider(names_from = leiden_enrichment,
              values_from = n,
              values_fill = 1) |>
  # mutate(fraction_treg = Treg / (`CD4 Naive` + `CD4 TCM` + `CD8 TEM` + NK + Treg)) |> 
  mutate(fraction_cd8tem_ibs = sensitive / (resistant + sensitive + unenriched)) |> 
  left_join(clinical_flow_data |> 
              filter(population == "CD8"),
            by = join_by(patient, timepoint_merged_2)) |> 
  mutate(abs_cd8tem_ibs = abs_mm3 * fraction_cd8tem_ibs) |> 
  ggplot(aes(x = patient_type, y = abs_cd8tem_ibs, fill = patient_type, color = patient_type)) +
  ...
  labs(y = "CD8 TEM_IBS:  Cells/mm<sup>3</sup>", 
       color = NULL, 
       fill = NULL, 
       x = NULL) +
