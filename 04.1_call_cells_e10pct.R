...
# redo the genotype assignment but allow some error
...
mt_match_func <- function(sample_fps,sample_names,fastas,i) {
  cmd <-
    paste0(
      "cutadapt -g file:",
      fastas[i],
      " --discard-untrimmed -e 0.1 --overlap 38 --action=trim -o working_files/btk_fastqs/mt_fastq_cb_trimmed/",
      sample_names[i],
...
wt_match_func <- function(sample_fps,sample_names,fastas,i) {
  cmd <-
    paste0(
      "cutadapt -g file:",
      fastas[i],
      " --discard-untrimmed -e 0.1 --overlap 38 --action=trim -o working_files/btk_fastqs/wt_fastq_cb_trimmed/",
      sample_names[i],
...
cell_calls_e10pct<-bind_rows(joined_info) %>% dplyr::rename(cb_pt_timepoint = cell_collapsed)
